...
const loadDocuments = async () => {
  ...
  try {
    const response = await doctorApi.getDocuments(props.namespace)
...
const deleteDocument = async () => {
  ...
  try {
    const response = await doctorApi.deleteDocument(props.namespace, deleteDocumentId.value)
    if (response.code === 200) {
      // 重新加载数据
      loadDocuments()
      // 关闭对话框
...
<template>
  <v-card class="document-list-card h-100">
    ...
    <delete-confirm-dialog
      ...
      title="确认删除文档"
      :content="`确定要删除文档『${deleteDocumentId}』吗？此操作无法撤销。`"
