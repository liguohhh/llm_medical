...
const loadNamespaces = async () => {
  ...
  try {
    const response = await doctorApi.getNamespaces()
...
const deleteNamespace = async () => {
  ...
  try {
    const response = await doctorApi.deleteNamespace(deleteNamespaceValue.value)
    if (response.code === 200) {
      // 重新加载数据
      loadNamespaces()
...
const createNamespace = async () => {
  ...
  try {
    const response = await doctorApi.createNamespace(newNamespace.value.trim())
    if (response.code === 200) {
      // 重新加载数据
...
<template>
  <v-card class="namespace-list-card h-100">
    ...
    <delete-confirm-dialog
      ...
      title="确认删除命名空间"
      :content="`确定要删除命名空间『${deleteNamespaceValue}』吗？此操作将删除该命名空间下的所有文档，无法撤销。`"
