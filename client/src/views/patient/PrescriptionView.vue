...
const loadPrescription = async () => {
  ...
  try {
    const response = await patientApi.getPrescriptionDetail(prescriptionId.value)
...
const goToPrescriptions = () => {
  router.push({ name: 'patientPrescriptions' })
}
      ...
      <template v-else-if="prescription">
        <v-card-text class="prescription-content">
          ...
          <v-card variant="outlined" class="prescription-content-card">
            ...
            <v-card-text>
              <div v-if="prescription.reviewStatus === 1">
              ...
              <div v-else-if="prescription.reviewStatus === 0" class="text-center py-4">
                <v-icon color="warning" size="large" class="mb-2">mdi-lock-clock</v-icon>
                <div class="text-h6">处方等待审核中</div>
                <div class="text-body-2 mb-4">处方内容将在医生审核通过后显示</div>
                <v-card class="mt-4 bg-grey-lighten-4 masked-content">
              ...
              <div v-else class="text-center py-4">
                <v-icon color="error" size="large" class="mb-2">mdi-lock</v-icon>
                <div class="text-h6">处方未通过审核</div>
                <div class="text-body-2">请联系医生了解详情</div>
              </div>
            </v-card-text>
          </v-card>
        </v-card-text>
